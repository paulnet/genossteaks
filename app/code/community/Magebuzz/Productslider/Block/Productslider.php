...
class Magebuzz_Productslider_Block_Productslider extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function __construct() {
		...
		if(!$this->getProductCollection()){
			...
			/*----Start SP Codez---*/
				case 'deal':
					$this->setProductCollection($this->getDealProducts());
					$this->setPageTitle('Deals of the Day');
					break;
			/*----End SP Codez---*/
				default:
					$this->setProductCollection($this->getRandomProducts());
	...
	public function getDealProducts() {
		$_limit = $this->getNumProduct();
		$deal_category = Mage::getStoreConfig('productslider/product_setting/deal_category');
		$collection = Mage::getResourceModel('catalog/product_collection')
								->addAttributeToSelect(Mage::getSingleton('catalog/config')->getProductAttributes())
								->addTaxPercents()
								->addStoreFilter();	
		...
		if($deal_category ){
			$currentCategory = Mage::getModel('catalog/category')->load($deal_category);
			$collection->addCategoryFilter($currentCategory);
		}
		$collection->setPageSize($_limit);
		return $collection;	
